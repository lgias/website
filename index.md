---
layout: page
title: Apps and connected hardware
---

LGIAS builds apps and connected hardware. Today, we build for water polo: tools that help parents, fans, and coaches film and follow the game, and help referees stay on top of the clock.

<p style="text-align: center; margin: 2rem 0 0.5rem;"><img src="/assets/watermark.png" alt="Water Polo Suite" style="width: 140px; max-width: 40%; height: auto;"></p>

## Water Polo Suite

Our water polo apps and hardware, built to work together:

<div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 1.75rem; margin: 1.75rem 0;">
<div style="width: 170px; text-align: center;">
<a href="/wpboard"><img src="/assets/wpboard.png" alt="Water Polo Board" style="display: block; width: 96px; height: 96px; border-radius: 22%; margin: 0 auto 0.6rem;"></a>
<a href="/wpboard"><strong>Water Polo Board</strong></a>
<p style="font-size: 0.9rem; color: #555; margin: 0.3rem 0 0;">A small wireless receiver that brings the pool's scoreboard timing to your phone.</p>
</div>
<div style="width: 170px; text-align: center;">
<a href="/wpcam"><img src="/assets/camera-icon.png" alt="Water Polo Camera" style="display: block; width: 96px; height: 96px; border-radius: 22%; margin: 0 auto 0.6rem;"></a>
<a href="/wpcam"><strong>Water Polo Camera</strong></a>
<p style="font-size: 0.9rem; color: #555; margin: 0.3rem 0 0;">Film a game with the live score and clock right on the video.</p>
</div>
<div style="width: 170px; text-align: center;">
<a href="/wprefassist"><img src="/assets/referee-icon.png" alt="Water Polo Referee Companion" style="display: block; width: 96px; height: 96px; border-radius: 22%; margin: 0 auto 0.6rem;"></a>
<a href="/wprefassist"><strong>Water Polo Referee Companion</strong></a>
<p style="font-size: 0.9rem; color: #555; margin: 0.3rem 0 0;">Clock and score cues in a referee's headset.</p>
</div>
</div>
